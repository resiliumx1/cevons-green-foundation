...
import {
  ...
  Trash2, Container, Droplet, Cylinder, Recycle, Waves, FileText, PackageX, Home, MoreHorizontal,
} from "lucide-react";
...
type ServiceKey =
  | "dumpster" | "skip" | "toilet" | "septic" | "oil" | "scrap" | "wastewater"
  | "garbage" | "shred" | "destruction" | "other";

const SERVICES: { key: ServiceKey; name: string; desc: string; icon: any; priority: boolean; specialist?: boolean }[] = [
  { key: "dumpster", name: "Dumpster Rental", desc: "Short or long term dumpster solutions.", icon: Trash2, priority: true },
  { key: "skip", name: "Skip Bin Rental", desc: "Various sizes for any project.", icon: Container, priority: true },
  { key: "toilet", name: "Portable Toilet Rental", desc: "Clean, hygienic toilet units.", icon: Droplet, priority: true },
  { key: "septic", name: "Septic Tank Clearance", desc: "Safe and efficient pumping.", icon: Waves, priority: true },
  { key: "oil", name: "Waste Oil Disposal", desc: "Compliant oil collection.", icon: Cylinder, priority: true, specialist: true },
  { key: "scrap", name: "Scrap Metal Collection", desc: "Pickup or drop-off available.", icon: Recycle, priority: true },
  { key: "wastewater", name: "Wastewater Treatment", desc: "Industrial wastewater handling.", icon: Waves, priority: true, specialist: true },
  { key: "garbage", name: "Garbage Collection", desc: "Scheduled household & business pickup.", icon: Home, priority: false },
  { key: "shred", name: "Document Shredding", desc: "Secure on-site or off-site shredding.", icon: FileText, priority: false },
  { key: "destruction", name: "Product Destruction", desc: "Certified product destruction.", icon: PackageX, priority: false, specialist: true },
  { key: "other", name: "Other Service", desc: "Tell us what you need.", icon: MoreHorizontal, priority: false },
];

const DIRECT_BOOKABLE: ServiceKey[] = ["dumpster", "skip", "toilet", "septic", "garbage", "shred"];
...
function RequestServicePage() {
  ...
  const selected = SERVICES.find((s) => s.key === data.service);
  const isSpecialist = !!selected?.specialist;
  const isBookable = selected ? DIRECT_BOOKABLE.includes(selected.key) : false;
  ...
  function validate(): boolean {
    ...
    if (step === 4 && !data.confirm) e.confirm = "Please confirm before submitting.";
    setErrors(e);
    ...
    <SiteLayout>
      ...
      <section className="container mx-auto px-4 py-10 md:py-14">
        <div className="grid lg:grid-cols-[1fr_320px] gap-8">
          ...
          <div>
            <Stepper step={step} />
            <div className="mt-8 rounded-2xl border border-border bg-card shadow-sm p-6 md:p-8 animate-fade-in" key={step}>
              {step === 0 && <StepService data={data} setData={setData} error={errors.service} />}
              {step === 1 && <StepDetails service={selected?.key ?? null} details={data.details} setDetail={setDetail} files={data.files} onFiles={onFiles} removeFile={removeFile} />}
              {step === 2 && <StepSchedule isBookable={isBookable} isSpecialist={isSpecialist} schedule={data.schedule} setSchedule={setSchedule} />}
              {step === 3 && <StepInfo info={data.info} setInfo={setInfo} errors={errors} />}
              {step === 4 && <StepReview data={data} selected={selected ?? null} isBookable={isBookable} confirm={data.confirm} setConfirm={(v) => setData((d) => ({ ...d, confirm: v }))} error={errors.confirm} />}
        ...
        return (
          <li key={label} className="flex-1 flex items-center gap-2 min-w-0">
            <div className={cn(
              ...
              completed && "bg-[#006B35] border-[#006B35] text-white",
              active && "bg-[#006B35] border-[#006B35] text-white",
...
function StepService({ data, setData, error }: { data: FormData; setData: (f: FormData) => void; error?: string }) {
  ...
  const other = SERVICES.filter((s) => !s.priority);
  return (
    <div>
      ...
      <h3 className="mt-6 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Priority Services</h3>
      <Grid items={priority} value={data.service} onSelect={(k) => setData({ ...data, service: k })} />

      <h3 className="mt-8 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Other Services</h3>
      <Grid items={other} value={data.service} onSelect={(k) => setData({ ...data, service: k })} />
    </div>
  );
}

function Grid({ items, value, onSelect }: { items: typeof SERVICES; value: ServiceKey | null; onSelect: (k: ServiceKey) => void }) {
  return (
        ...
        return (
          <button
            ...
            className={cn(
              ...
              active ? "border-[#006B35] bg-[#006B35]/5 scale-[1.01]" : "border-border hover:border-[#006B35]/40",
            )}
          >
        ...
        {service === "dumpster" || service === "skip" ? (
          ...
          <>
            <Field label="Use case">
              <Select value={details.useCase ?? ""} onValueChange={(v) => setDetail("useCase", v)}>
                ...
                <SelectContent>
                  {["Event", "Construction Site", "Commercial Site", "Public Facility", "Other"].map((x) => <SelectItem key={x} value={x}>{x}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
            ...
            <Field label="Servicing frequency"><Input className="h-12" value={details.freq ?? ""} onChange={(e) => setDetail("freq", e.target.value)} placeholder="e.g. weekly" /></Field>
          ...
          <>
            <Field label="Customer type">
              <Select value={details.customerType ?? ""} onValueChange={(v) => setDetail("customerType", v)}>
                ...
                <SelectContent>{["Residential", "Commercial", "Industrial"].map((x) => <SelectItem key={x} value={x}>{x}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
            <Field label="Tank type (if known)"><Input className="h-12" value={details.tank ?? ""} onChange={(e) => setDetail("tank", e.target.value)} /></Field>
          ...
          <>
            <Field label="Oil type">
              <Select value={details.oilType ?? ""} onValueChange={(v) => setDetail("oilType", v)}>
                <SelectTrigger className="h-12"><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>{["Cooking Oil", "Lube Oil", "Waste Oil", "Other"].map((x) => <SelectItem key={x} value={x}>{x}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
            <Field label="Estimated quantity"><Input className="h-12" value={details.qty ?? ""} onChange={(e) => setDetail("qty", e.target.value)} placeholder="e.g. 200 L" /></Field>
            <Field label="Container type"><Input className="h-12" value={details.container ?? ""} onChange={(e) => setDetail("container", e.target.value)} placeholder="Drums, IBC, tank" /></Field>
            <Field label="Pickup frequency"><Input className="h-12" value={details.freq ?? ""} onChange={(e) => setDetail("freq", e.target.value)} placeholder="One-time, monthly, etc." /></Field>
            <Field label="Business type"><Input className="h-12" value={details.business ?? ""} onChange={(e) => setDetail("business", e.target.value)} /></Field>
            <Field label="Documentation required">
              <Select value={details.docs ?? ""} onValueChange={(v) => setDetail("docs", v)}>
                <SelectTrigger className="h-12"><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>{["Yes", "No"].map((x) => <SelectItem key={x} value={x}>{x}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
          </>
        ) : service === "scrap" ? (
          <>
            <Field label="Metal type"><Input className="h-12" value={details.metal ?? ""} onChange={(e) => setDetail("metal", e.target.value)} placeholder="Iron, copper, aluminum, mixed…" /></Field>
            <Field label="Estimated quantity"><Input className="h-12" value={details.qty ?? ""} onChange={(e) => setDetail("qty", e.target.value)} /></Field>
            <Field label="Pickup or drop-off">
              <Select value={details.mode ?? ""} onValueChange={(v) => setDetail("mode", v)}>
                <SelectTrigger className="h-12"><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>{["Pickup", "Drop-off"].map((x) => <SelectItem key={x} value={x}>{x}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
            <Field label="Loading access"><Input className="h-12" value={details.loading ?? ""} onChange={(e) => setDetail("loading", e.target.value)} /></Field>
            <Field label="Location notes" full><Textarea value={details.location ?? ""} onChange={(e) => setDetail("location", e.target.value)} /></Field>
          </>
        ) : service === "wastewater" ? (
          <>
            <Field label="Wastewater source"><Input className="h-12" value={details.source ?? ""} onChange={(e) => setDetail("source", e.target.value)} /></Field>
            <Field label="Estimated volume"><Input className="h-12" value={details.volume ?? ""} onChange={(e) => setDetail("volume", e.target.value)} /></Field>
            <Field label="Waste type / details" full><Textarea value={details.wasteDetails ?? ""} onChange={(e) => setDetail("wasteDetails", e.target.value)} /></Field>
            <Field label="Site type"><Input className="h-12" value={details.site ?? ""} onChange={(e) => setDetail("site", e.target.value)} /></Field>
          ...
          <>
            <Field label="Product type"><Input className="h-12" value={details.product ?? ""} onChange={(e) => setDetail("product", e.target.value)} /></Field>
            <Field label="Quantity"><Input className="h-12" value={details.qty ?? ""} onChange={(e) => setDetail("qty", e.target.value)} /></Field>
            <Field label="Reason for destruction" full><Textarea value={details.reason ?? ""} onChange={(e) => setDetail("reason", e.target.value)} /></Field>
            <Field label="Documentation required">
              <Select value={details.docs ?? ""} onValueChange={(v) => setDetail("docs", v)}>
                <SelectTrigger className="h-12"><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>{["Yes", "No"].map((x) => <SelectItem key={x} value={x}>{x}</SelectItem>)}</SelectContent>
            ...
            <Field label="Pickup location" full><Input className="h-12" value={details.location ?? ""} onChange={(e) => setDetail("location", e.target.value)} /></Field>
          </>
        ) : service === "shred" ? (
          <>
            ...
            <Field label="Preferred date"><Input type="date" className="h-12" value={details.date ?? ""} onChange={(e) => setDetail("date", e.target.value)} /></Field>
          ...
          <>
            ...
            <Field label="Pickup frequency">
              <Select value={details.freq ?? ""} onValueChange={(v) => setDetail("freq", v)}>
                <SelectTrigger className="h-12"><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>{["Weekly", "Twice weekly", "Daily", "Other"].map((x) => <SelectItem key={x} value={x}>{x}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
            ...
            <Field label="Additional notes" full><Textarea value={details.notes ?? ""} onChange={(e) => setDetail("notes", e.target.value)} /></Field>
          </>
        ) : service === "other" ? (
          <>
            <Field label="Describe the service needed" full><Textarea value={details.describe ?? ""} onChange={(e) => setDetail("describe", e.target.value)} /></Field>
            <Field label="Location" full><Input className="h-12" value={details.location ?? ""} onChange={(e) => setDetail("location", e.target.value)} /></Field>
          </>
        ) : (
          <div className="md:col-span-2 text-muted-foreground">Please select a service first.</div>
        )}
      </div>

      {/* Upload */}
      <div className="mt-8">
        <Label className="text-sm font-semibold">Upload Photos or Documents</Label>
        <label
          htmlFor="file-upload"
...
function StepSchedule({
  isBookable, isSpecialist, schedule, setSchedule,
}: {
  isBookable: boolean; isSpecialist: boolean;
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground mt-1">
        {isBookable ? "Pick your preferred date and time window." : "Tell us when you'd like to be contacted."}
      </p>
      ...
      <div className="mt-6 grid md:grid-cols-2 gap-4">
        {isBookable ? (
          <>
            <Field label="Preferred date"><Input type="date" className="h-12" value={schedule.date} onChange={(e) => setSchedule("date", e.target.value)} /></Field>
          ...
          <>
            ...
            <Field label="Preferred service timeframe" full>
              <Input className="h-12" value={schedule.timeframe} onChange={(e) => setSchedule("timeframe", e.target.value)} placeholder="e.g. next 2 weeks" />
            </Field>
          </>
        )}
      </div>
...
/* ---------------- Step 5 ---------------- */
function StepReview({
  data, selected, isBookable, confirm, setConfirm, error,
}: {
  data: FormData; selected: (typeof SERVICES)[number] | null; isBookable: boolean;
  ...
  return (
    <div>
      ...
      <div className="mt-6 space-y-4">
        ...
        <ReviewBlock title={isBookable ? "Schedule" : "Contact preference"}>
