...
export function ProcessSteps({ steps }: { steps: ProcessStep[] }) {
  ...
  return (
    <div className="relative">
      {/* One DOM copy of the steps; CSS reflows it across breakpoints:
            • Mobile  (<md):  left-rail vertical timeline
            • Tablet  (md):   2-up centered cards
            • Desktop (lg+):  horizontal spine + step columns          */}
      <ol
        ref={mobileWrapRef}
        className="relative flex flex-col gap-8 md:grid md:grid-cols-2 md:gap-x-6 md:gap-y-10 lg:grid-cols-6 lg:gap-x-4"
      >
        {/* Mobile rail track + progress fill, inside the circle column */}
        <div
          aria-hidden="true"
          className="md:hidden absolute w-[2px] pointer-events-none"
          style={{
            top: rail.top,
            height: rail.height,
            left: CIRCLE_HALF_MOBILE - 1,
            zIndex: 0,
          }}
        >
        ...
        {/* Desktop horizontal connector aligned with circle centers */}
        <div
          aria-hidden="true"
        ...
        {steps.map((s, i) => {
          ...
          return (
            <motion.li
              key={s.key}
              className="relative grid grid-cols-[56px_minmax(0,1fr)] gap-4 items-start md:flex md:flex-col md:items-center md:text-center md:gap-0 md:px-2"
              style={{ zIndex: 1 }}
              custom={i}
              initial="hidden"
              whileInView="show"
            ...
            >
              <div
                ...
                className="relative size-14 md:size-16 rounded-full flex items-center justify-center shadow-soft shrink-0"
                style={{
              ...
              <div className="min-w-0 pt-1 md:pt-0 md:flex md:flex-col md:items-center">
                <p className="text-[11px] font-bold tracking-wider text-[var(--text-eyebrow)] uppercase md:mt-3">
                  {s.step} {s.index}
                </p>
                <h3 className="text-base font-bold mt-0.5 md:mt-1 text-cevons-dark">
                  {s.title}
                </h3>
                <p className="text-sm lg:text-xs text-cevons-muted mt-1 md:mt-1.5 leading-relaxed md:max-w-[28ch] lg:max-w-[22ch]">
                  {s.body}
                </p>
              </div>
            </motion.li>
